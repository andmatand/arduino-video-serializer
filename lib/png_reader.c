...
coolpng read_png_file(char *filename) {
    ...
    // Make a new coolpng struct
    coolpng myPng;

    // Give the coolpng the values
    ...
    myPng.row_pointers = (png_bytep*)malloc(sizeof(png_bytep) * myPng.height);
    for(int y = 0; y < myPng.height; y++) {
        myPng.row_pointers[y] = (png_byte*)malloc(png_get_rowbytes(png,info));
    }

    png_read_image(png, myPng.row_pointers);
